...
library(survey)
iterations <- 5000 # the number of iterations to run for simulations
```
...
and for error estimates (less than 2 % relative bias):
...
The ratio estimator is defined by the following workfloe:
...
And a simulation routine is defined, similar to the one above, but using the landings-ratio estimator in stead of the simple design based.


```{r simulateRatioEst}
simulateRatioEst <- function(iterations=1000, n=80, domain=T){
...
```{r simDomainsRatio}
timestampStart <- Sys.time()
simStatsAgeDomainsRatio <- simulateRatioEst(iterations, domain=T)
timestampEnd <- Sys.time()
difftime(timestampEnd, timestampStart)
```
...
First we inspect the mean absoulte error for the point estimators over the simulations. In accordance with expectation, we note a slight reduction in error for abundance and total weight estimates in large domains, when using the landings ratio estimator. Note that this is data from two independent simulations, which explains small deviations in mean weight estimates, which are identical for the two methods.
...
ggplot2::ggplot(comp[Param=="Abundance",]) + ggplot2::geom_col(ggplot2::aes(x=Age, y=abs(error), fill=Estimator), position="dodge") + ggplot2::ggtitle("Abundance")
ggplot2::ggplot(comp[Param=="TotalWeight",]) + ggplot2::geom_col(ggplot2::aes(x=Age, y=abs(error), fill=Estimator), position="dodge")  + ggplot2::ggtitle("Total Weight")
ggplot2::ggplot(comp[Param=="MeanWeight",]) + ggplot2::geom_col(ggplot2::aes(x=Age, y=abs(error), fill=Estimator), position="dodge")  + ggplot2::ggtitle("Mean Weight")
```
The ratio estimate is however slightly biased:
...
```
As is the estimate of the error:
